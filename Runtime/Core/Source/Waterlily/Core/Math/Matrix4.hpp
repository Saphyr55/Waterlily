...
namespace Wl
{
    ...
    struct Matrix4
    {
        ...
        constexpr Matrix4(const Vector4<R>& col_1, const Vector4<R>& col_2, const Vector4<R>& col_3, const Vector4<R>& col_4);

        static constexpr Matrix4 Identity();

        static constexpr Matrix4 Translate(const Matrix4<R>& mat, const Vector3<R>& vec);
    ...
    constexpr Matrix4<R> Matrix4<R>::Rotate(const Matrix4<R>& mat, const Real auto& theta, const Vector3<R>& vec)
    {
        ...
        return mat * Matrix4<R>(c + v.x * v.x * (1 - c),
                                v.x * v.y * (1 - c) - v.z * s,
                                v.x * v.z * (1 - c) + v.y * s,
                                0,
                                v.x * v.y * (1 - c) + v.z * s,
    ...
    constexpr Matrix4<R>::Matrix4(R m11,
                                  ...
                                  R m44)
        : Matrix4(
                  Vector4<R>(m11, m12, m13, m14),
                  Vector4<R>(m21, m22, m23, m24),
                  Vector4<R>(m31, m32, m33, m34),
                  Vector4<R>(m41, m42, m43, m44))
    {
    }
